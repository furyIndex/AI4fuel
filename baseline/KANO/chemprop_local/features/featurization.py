...
ele2emb = pickle.load(open('other_baseline/KANO/initial/ele2emb.pkl','rb'))
def ele_features(ele):
    fele = ele2emb[ele]
    return fele.tolist()

rel2emb = pickle.load(open('other_baseline/KANO/initial/rel2emb.pkl','rb'))
def relation_features(e1,e2):
    frel = rel2emb[(e1,e2)]
    return frel.tolist()

with open('other_baseline/KANO/chemprop_local/data/funcgroup.txt', "r") as f:
...
fg2emb = pickle.load(open('other_baseline/KANO/initial/fg2emb.pkl', 'rb'))
def match_fg(mol):
    fg_emb = [[1] * 133]
    pad_fg = [[0] * 133]
    for sm in smart:
